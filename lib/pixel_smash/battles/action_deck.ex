defmodule PixelSmash.Battles.ActionDeck do
  ...
  def actions(stats, fighter, opponent) do
    {_, attack_actions} = Enum.reduce(@attack_multiplier, {stats, []}, fn {color_count, multiplier}, {stats, actions} ->
  ...
  defp expand_to_list(map) do
    Enum.flat_map(map, fn {key, value} -> List.duplicate(key, value) end)
  end

  defp reduce_stats(stats, color_count, reductions_count \\ 0) do
    reduced_stats = Enum.reduce_while(color_count, stats, fn {color, count}, stats ->
